...
(after! org-roam
  ...
  ;; The repo documentation mirror under 02_areas/repos/<project>/docs/ is
  ;; machine-generated by `.scripts/repo_docs_sync' and tagged `:generated:'.
  ;; It is a few hundred nodes for three projects and would grow past a
  ;; thousand if cmacs were added, which is enough to bury every hand-written
  ;; note in the completion list.  The nodes stay in the database -- inbound
  ;; [[id:...]] links, backlinks and the graph view all keep working -- they are
  ;; simply not offered as completion candidates unless asked for.
  (defcustom zach/org-roam-hide-generated t
    "When non-nil, omit `:generated:' nodes from `org-roam-node-read' candidates.
A prefix argument overrides this for a single call."
    :type 'boolean
    :group 'org-roam)

  (defun zach/org-roam-node-not-generated-p (node)
    "Return non-nil unless NODE carries the `generated' tag."
    (not (member "generated" (org-roam-node-tags node))))

  (defun zach/org-roam-node-read-hide-generated (fn &optional initial-input filter-fn &rest args)
    "Around advice for FN (`org-roam-node-read') hiding generated nodes.
An explicitly supplied FILTER-FN always wins, as does a prefix argument."
    (apply fn initial-input
           (or filter-fn
               (and zach/org-roam-hide-generated
                    (not current-prefix-arg)
                    #'zach/org-roam-node-not-generated-p))
           args))

  (advice-add 'org-roam-node-read :around #'zach/org-roam-node-read-hide-generated))
