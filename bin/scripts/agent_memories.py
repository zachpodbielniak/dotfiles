...
def _ollama_embed_sync(text: str) -> Optional[list[float]]:
    """
    Generate an embedding vector via Ollama HTTP API (synchronous).

    Uses stdlib urllib so no extra deps required. Returns None on any
    failure (ollama down, timeout, model not loaded, etc.).

    Args:
        text: The text to embed

    Returns:
        768-dimension float vector, or None on failure
    """
    url: str = f"{OLLAMA_URL}/api/embed"
...
async def generate_embedding(text: str) -> Optional[list[float]]:
    """
    Generate embedding via Ollama, non-blocking.

    Runs the synchronous HTTP call in a thread executor so it doesn't
    ...
    return await loop.run_in_executor(None, _ollama_embed_sync, text)
...
class MemoryManager:
    ...
    async def add_memory(
        ...
        memory_id: str = str(row["id"])

        # Generate embedding async (non-fatal if ollama is unavailable)
        embed_text: str = f"{summary or ''} {content}"
        embedding: Optional[list[float]] = await generate_embedding(embed_text)
        if embedding:
    ...
    async def update_memory(self, memory_id: str, **kwargs) -> bool:
        ...
        if "content" in kwargs or "summary" in kwargs:
            row = await self.pool.fetchrow(
                "SELECT content, summary FROM memories WHERE id = $1", full_id
            )
            if row:
                embed_text: str = f"{row['summary'] or ''} {row['content']}"
                embedding: Optional[list[float]] = await generate_embedding(embed_text)
                if embedding:
                    await self.pool.execute(
                        "UPDATE memories SET embedding = $1 WHERE id = $2",
    ...
    async def search_memories(
        ...
        # Weight confidence into ranking
        confidence_rank: str = f"({rank_expr}) * COALESCE(confidence, 1.0)"

        sql: str = f"""
            SELECT *, {rank_expr} as relevance
            FROM memories
            WHERE {where}
            ORDER BY is_pinned DESC, ({confidence_rank}) DESC, created_at DESC
            LIMIT ${idx}
        """
    ...
    async def semantic_search(
        ...
        the most similar memories using the HNSW index. Falls back to
        fuzzy search if ollama is unavailable or no embeddings exist.
        ...
        Returns:
            List of matching memory dicts, ranked by cosine similarity
        """
        query_embedding: Optional[list[float]] = await generate_embedding(query)
        if query_embedding is None:
            print(
                _yellow("Warning: ollama unavailable, falling back to fuzzy search"),
                file=sys.stderr,
            )
        ...
        return [dict(r) for r in rows if r["similarity"] >= 0.5]
...
def format_memory_detail(mem: dict) -> str:
    ...
    lines.append(f"  {_cyan('Confidence:')}  {mem.get('confidence', 1.0):.2f}")

    expires: str = "-"
    if mem.get("expires_at"):
        exp_dt: datetime = mem["expires_at"]
        expires = exp_dt.strftime("%Y-%m-%d %H:%M:%S")
        if exp_dt < datetime.now(timezone.utc):
            expires += _red(" (EXPIRED)")
    lines.append(f"  {_cyan('Expires:')}     {expires}")
    has_embed: str = "yes" if mem.get("embedding") else "no"
    lines.append(f"  {_cyan('Embedding:')}   {has_embed}")
...
def format_memory_table(memories: list[dict], columns: Optional[list[str]] = None) -> str:
    ...
    col_widths: dict[str, int] = {
        "id": 8, "similarity": 6, "category": 12, "subcategory": 12,
        "importance": 8, "summary": 50, "source": 15, "tags": 25,
        "created_at": 10, "updated_at": 10, "access_count": 6, "is_pinned": 3,
    }
    ...
    for mem in memories:
        ...
        for col in cols:
            ...
            if col == "id":
                ...
                val = f"{float(val):.2f}" if val is not None else ""
            elif col == "tags":
...
def _parse_expiry(value: str) -> datetime:
    """
    Parse a relative or absolute expiry string into a datetime.

    Relative formats: 7d (days), 2w (weeks), 3m (months), 1y (years)
    Absolute: ISO 8601 date or datetime string
    """
    from datetime import timedelta

    match: Optional[re.Match] = re.match(r"^(\d+)([dwmy])$", value.strip())
    if match:
    ...
    try:
        ...
        print(_red(f"Invalid expiry format: '{value}'"), file=sys.stderr)
        print("Use relative (7d, 2w, 3m, 1y) or ISO date (2026-03-01)", file=sys.stderr)
        sys.exit(1)
...
async def cmd_search(mgr: MemoryManager, args: argparse.Namespace) -> None:
    ...
    if semantic:
        results: list[dict] = await mgr.semantic_search(
            query=args.query,
            category=args.category,
            include_archived=args.include_archived,
            limit=args.limit,
    ...
    fmt: str = args.format
    # Show similarity score column for semantic searches
    sem_cols: Optional[list[str]] = None
    if semantic and results:
        sem_cols = ["id", "similarity", "category", "importance", "summary", "tags"]
    ...
    if fmt == "json":
        ...
        print(format_memory_table(results, columns=sem_cols))
...
async def cmd_prune(mgr: MemoryManager, args: argparse.Namespace) -> None:
    """Handle the 'prune' subcommand — archive or delete expired memories."""
    # Count expired memories
    expired_count: int = await mgr.pool.fetchval(
    ...
    if expired_count == 0:
        print("No expired memories to prune.")
        return

    if args.dry_run:
        print(f"Would prune {expired_count} expired memories:")
        rows = await mgr.pool.fetch(
            "SELECT id, summary, expires_at FROM memories "
            "WHERE expires_at IS NOT NULL AND expires_at < CURRENT_TIMESTAMP "
            "AND is_archived = FALSE "
            "ORDER BY expires_at ASC"
        )
        for row in rows:
            exp: str = row["expires_at"].strftime("%Y-%m-%d %H:%M")
            print(f"  [{str(row['id'])[:8]}] expired {exp}: {row['summary'] or '(no summary)'}")
        return

    if args.delete:
        # Permanently delete expired memories
        result: str = await mgr.pool.execute(
            "DELETE FROM memories "
            "WHERE expires_at IS NOT NULL AND expires_at < CURRENT_TIMESTAMP"
        )
        count: int = int(result.split(" ")[-1])
        print(_green(f"Deleted {count} expired memories"))
    else:
        # Archive expired memories
        result = await mgr.pool.execute(
            "UPDATE memories SET is_archived = TRUE "
            "WHERE expires_at IS NOT NULL AND expires_at < CURRENT_TIMESTAMP "
            "AND is_archived = FALSE"
        )
        count = int(result.split(" ")[-1])
        print(_green(f"Archived {count} expired memories"))
...
async def cmd_backfill(mgr: MemoryManager, args: argparse.Namespace) -> None:
    """
    Generate embeddings for all memories that don't have one yet.

    Iterates through memories with NULL embedding, calls ollama for each,
    and updates the row. Prints progress as it goes. Skips archived by
    default unless --include-archived is set.
    """
    conditions: list[str] = ["embedding IS NULL"]
    if not getattr(args, "include_archived", False):
        conditions.append("is_archived = FALSE")

    where: str = " AND ".join(conditions)
    rows = await mgr.pool.fetch(
        f"SELECT id, content, summary FROM memories WHERE {where} ORDER BY created_at ASC"
    )
    ...
    print(f"Backfilling embeddings for {total} memories...")
    success: int = 0
    failed: int = 0
    ...
    for i, row in enumerate(rows, 1):
        ...
        if embedding:
            ...
            failed += 1
            if failed == 1:
                print(_yellow("Warning: ollama may be unavailable"), file=sys.stderr)

        # Progress every 10 or on last
        if i % 10 == 0 or i == total:
            print(f"  [{i}/{total}] {success} embedded, {failed} failed", end="\r")

    print(f"\n{_green(f'Done: {success} embedded')}, {_yellow(f'{failed} failed') if failed else '0 failed'}")
...
# =============================================================================
# Bootstrap — structured context injection for session start hooks
# =============================================================================

async def cmd_bootstrap(mgr: MemoryManager, args: argparse.Namespace) -> None:
    """
    Output structured memory context for AI session bootstrap.

    Pulls pinned, decisions, preferences, critical, and recent memories
    into a compact format (~500-800 tokens) suitable for injection via
    a SessionStart hook. Deduplicates by UUID across sections.
    """
    seen_ids: set[str] = set()
    sections: list[tuple[str, list[dict]]] = []
    recent_limit: int = args.limit if hasattr(args, "limit") else 5
    fmt: str = args.format if hasattr(args, "format") else "text"

    # 1. Pinned memories (all, non-archived)
    pinned: list[dict] = await mgr.list_memories(pinned=True, limit=100)
    sections.append(("PINNED", pinned))

    # 2. All decisions (non-archived)
    decisions: list[dict] = await mgr.list_memories(category="decision", limit=100)
    sections.append(("DECISIONS", decisions))

    # 3. All preferences (non-archived)
    preferences: list[dict] = await mgr.list_memories(category="preference", limit=100)
    sections.append(("PREFERENCES", preferences))

    # 4. All critical importance (non-archived)
    critical: list[dict] = await mgr.list_memories(importance="critical", limit=100)
    sections.append(("CRITICAL", critical))

    # 5. Recent from key categories
    for cat in ("project", "technical", "learning", "workflow"):
        recent: list[dict] = await mgr.list_memories(
            category=cat, limit=recent_limit
        )
        sections.append((f"RECENT ({cat})", recent))

    if fmt == "json":
        _bootstrap_json(sections, seen_ids)
    else:
        _bootstrap_text(sections, seen_ids)


def _bootstrap_format_line(mem: dict) -> str:
    """Format a single memory as a compact one-liner for bootstrap output."""
    mid: str = str(mem["id"])[:8]
...
def _bootstrap_text(
    ...
    """Output bootstrap context as compact structured text."""
    total: int = 0
    output_lines: list[str] = []

    for section_name, memories in sections:
        lines: list[str] = []
        for mem in memories:
            mid: str = str(mem["id"])
            if mid in seen_ids:
                continue
            seen_ids.add(mid)
            lines.append(_bootstrap_format_line(mem))
            total += 1
    ...
    if total == 0:
        print("=== AGENT MEMORIES BOOTSTRAP ===")
        print("No memories stored yet. Use 'agent_memories add' to create memories.")
        print("=== END BOOTSTRAP ===")
        return

    print("=== AGENT MEMORIES BOOTSTRAP ===")
    print(f"[{total} memories injected]")
...
def _bootstrap_json(
    ...
    """Output bootstrap context as JSON."""
    result: dict = {"sections": {}, "total": 0}
    total: int = 0
    ...
    for section_name, memories in sections:
        ...
        if entries:
            result["sections"][section_name] = entries
...
# =============================================================================
# MCP stdio server — Model Context Protocol over JSON-RPC 2.0
# =============================================================================

MCP_PROTOCOL_VERSION: str = "2024-11-05"
MCP_SERVER_NAME: str = "agent-memories"
MCP_SERVER_VERSION: str = "1.0.0"


def _mcp_serialize(obj) -> object:
    """JSON serializer for MCP responses — handles datetime, UUID, bytes."""
    if isinstance(obj, (datetime, date)):
        return obj.isoformat()
    if hasattr(obj, "hex"):  # UUID
        return str(obj)
...
def _mcp_tool_definitions() -> list[dict]:
    """Return the list of MCP tool definitions with JSON schemas."""
    category_enum: list[str] = CATEGORIES
    importance_enum: list[str] = IMPORTANCE_LEVELS

    return [
        {
            "name": "memory_add",
            "description": "Store a new memory in the persistent agent memory database.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "content": {"type": "string", "description": "The full memory content text"},
                    "summary": {"type": "string", "description": "Short one-line summary (auto-generated from content if omitted)"},
                    "category": {"type": "string", "enum": category_enum, "description": "Memory category (default: general)"},
                    "subcategory": {"type": "string", "description": "Subcategory for finer classification"},
                    "importance": {"type": "string", "enum": importance_enum, "description": "Importance level (default: normal)"},
                    "source": {"type": "string", "description": "Source of the memory (e.g., conversation, heartbeat)"},
                    "source_context": {"type": "string", "description": "Additional source context"},
                    "conversation_id": {"type": "string", "description": "Link to a specific conversation"},
                    "tags": {"type": "array", "items": {"type": "string"}, "description": "Tags for categorization"},
                    "related_to": {"type": "string", "description": "UUID (or prefix) of a related memory"},
                    "pin": {"type": "boolean", "description": "Pin this memory (default: false)"},
                    ...
                    "expires": {"type": "string", "description": "Expiry: relative (7d, 2w, 3m, 1y) or ISO date"},
                },
                "required": ["content"],
            },
        },
        {
            "name": "memory_show",
            "description": "Fetch and display a single memory by ID (full UUID or prefix).",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "id": {"type": "string", "description": "Memory ID (full UUID or prefix)"},
                },
                "required": ["id"],
            },
        },
        {
            "name": "memory_list",
            "description": "List memories with optional filtering, sorting, and pagination.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "category": {"type": "string", "enum": category_enum, "description": "Filter by category"},
                    "subcategory": {"type": "string", "description": "Filter by subcategory"},
                    "importance": {"type": "string", "enum": importance_enum, "description": "Filter by importance"},
                    "source": {"type": "string", "description": "Filter by source"},
                    "tags": {"type": "array", "items": {"type": "string"}, "description": "Filter by tags (all must match)"},
                    "pinned": {"type": "boolean", "description": "Show only pinned memories"},
                    "include_archived": {"type": "boolean", "description": "Include archived memories"},
                    "only_archived": {"type": "boolean", "description": "Show only archived memories"},
                    "limit": {"type": "integer", "description": "Max results (default: 50)"},
                    "offset": {"type": "integer", "description": "Pagination offset"},
                    "sort": {"type": "string", "description": "Sort column (default: created_at)"},
        ...
        },
        {
            "name": "memory_search",
            "description": (
                "Search memories using fuzzy (default), exact (tsvector), or semantic (pgvector+ollama) modes. "
                "Fuzzy: tolerates typos, partial matches. "
                "Exact: boolean operators (AND, |, -, :*). "
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "mode": {
                        ...
                        "description": "Search mode: fuzzy (default), exact (tsvector boolean), or semantic (embedding similarity)",
                    },
                    "category": {"type": "string", "enum": category_enum, "description": "Filter by category"},
                    "include_archived": {"type": "boolean", "description": "Include archived memories"},
        ...
        {
            "name": "memory_update",
            "description": "Update fields on an existing memory. Only provided fields are changed.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "id": {"type": "string", "description": "Memory ID (full UUID or prefix)"},
                    ...
                    "category": {"type": "string", "enum": category_enum, "description": "New category"},
                    "subcategory": {"type": "string", "description": "New subcategory"},
                    "importance": {"type": "string", "enum": importance_enum, "description": "New importance level"},
                    "source": {"type": "string", "description": "New source"},
                    "source_context": {"type": "string", "description": "New source context"},
                    "tags": {"type": "array", "items": {"type": "string"}, "description": "Replace tags array"},
                    "is_pinned": {"type": "boolean", "description": "Set pinned status"},
                    ...
                    "expires_at": {"type": "string", "description": "New expiry: relative (7d, 2w, 3m, 1y) or ISO date"},
                },
                "required": ["id"],
            },
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    "id": {"type": "string", "description": "Memory ID (full UUID or prefix)"},
                    "unarchive": {"type": "boolean", "description": "Unarchive instead of archive (default: false)"},
                },
                "required": ["id"],
            },
        },
        {
            "name": "memory_pin",
            "description": "Pin or unpin a memory. Pinned memories rank first in search results and appear in bootstrap.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "id": {"type": "string", "description": "Memory ID (full UUID or prefix)"},
                    "unpin": {"type": "boolean", "description": "Unpin instead of pin (default: false)"},
                },
                "required": ["id"],
            },
        },
        {
            "name": "memory_delete",
            "description": "Permanently delete a memory. This cannot be undone.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "id": {"type": "string", "description": "Memory ID (full UUID or prefix)"},
        ...
        {
            "name": "memory_tag",
            "description": "Add or remove tags on a memory.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "id": {"type": "string", "description": "Memory ID (full UUID or prefix)"},
                    "tags": {"type": "array", "items": {"type": "string"}, "description": "Tags to add or remove"},
                    "remove": {"type": "boolean", "description": "Remove the specified tags instead of adding (default: false)"},
                },
                "required": ["id", "tags"],
            },
        },
        {
            "name": "memory_stats",
            "description": "Get aggregate statistics about the memory store (counts by category, importance, top tags, etc).",
            "inputSchema": {
                "type": "object",
                "properties": {},
            },
        ...
        {
            "name": "memory_export",
            "description": "Export all memories as a JSON array.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "include_archived": {"type": "boolean", "description": "Include archived memories (default: true)"},
                },
            },
        },
        {
            "name": "memory_import",
            "description": "Import memories from a JSON array of memory objects.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "memories": {
        ...
        {
            "name": "memory_prune",
            "description": "Archive or permanently delete expired memories.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "delete": {"type": "boolean", "description": "Permanently delete instead of archiving (default: false)"},
                    "dry_run": {"type": "boolean", "description": "Preview what would be pruned without executing (default: false)"},
                },
            },
        },
        {
            "name": "memory_backfill",
            "description": "Generate embeddings for all memories that are missing them (requires ollama running).",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        },
        {
            ...
            "description": "Get structured memory context for AI session bootstrap. Returns pinned, decisions, preferences, critical, and recent memories.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "format": {"type": "string", "enum": ["text", "json"], "description": "Output format (default: text)"},
                    "limit": {"type": "integer", "description": "Number of recent memories per category (default: 5)"},
                },
            },
        },
        {
            "name": "memory_bootstrap_slim",
            "description": "Get minimal memory context for AI session bootstrap. Returns only pinned and critical memories. Lighter than memory_bootstrap — use during compaction or when context budget is tight.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "format": {"type": "string", "enum": ["text", "json"], "description": "Output format (default: text)"},
...
async def _mcp_handle_tool_call(mgr: MemoryManager, name: str, arguments: dict) -> dict:
    """
    Dispatch an MCP tool call to the appropriate MemoryManager method.

    Args:
        mgr: Initialized MemoryManager instance
        name: Tool name (e.g., "memory_add")
        arguments: Tool arguments dict

    Returns:
        MCP result dict with "content" list
    """
    ...
    def _json_result(data) -> dict:
        return _text_result(json.dumps(data, indent=2, default=_mcp_serialize))

    try:
        if name == "memory_add":
            expires_at: Optional[datetime] = None
            if arguments.get("expires"):
                expires_at = _parse_expiry(arguments["expires"])
            ...
            memory_id: str = await mgr.add_memory(
                ...
                pinned=arguments.get("pin", False),
                metadata=arguments.get("metadata"),
                confidence=arguments.get("confidence", 1.0),
                expires_at=expires_at,
            )
            return _json_result({"status": "created", "id": memory_id})

        elif name == "memory_show":
            mem: Optional[dict] = await mgr.get_memory(arguments["id"])
            if not mem:
                return _text_result(f"Memory not found: {arguments['id']}", is_error=True)
            return _json_result(mem)
            ...
            memories: list[dict] = await mgr.list_memories(
                ...
                source=arguments.get("source"),
                pinned=True if arguments.get("pinned") else None,
                include_archived=arguments.get("include_archived", False),
                only_archived=arguments.get("only_archived", False),
                limit=arguments.get("limit", 50),
                offset=arguments.get("offset", 0),
                order_by=arguments.get("sort", "created_at"),
                descending=not arguments.get("ascending", False),
            )
            return _json_result(memories)

        elif name == "memory_search":
            mode: str = arguments.get("mode", "fuzzy")
            ...
            mid: str = arguments.pop("id")
            update_fields: dict = {}
            for key in ("content", "summary", "category", "subcategory",
                        "importance", "source", "source_context", "tags",
                        "is_pinned", "is_archived", "metadata", "confidence"):
                if key in arguments:
                    update_fields[key] = arguments[key]
            if "expires_at" in arguments:
                update_fields["expires_at"] = _parse_expiry(arguments["expires_at"])
            if not update_fields:
                return _text_result("No fields to update", is_error=True)
            updated: bool = await mgr.update_memory(mid, **update_fields)
            if updated:
                return _json_result({"status": "updated", "id": mid})
            return _text_result(f"Memory not found: {mid}", is_error=True)

        elif name == "memory_archive":
            value: bool = not arguments.get("unarchive", False)
            updated = await mgr.update_memory(arguments["id"], is_archived=value)
            if updated:
                action: str = "unarchived" if arguments.get("unarchive") else "archived"
                return _json_result({"status": action, "id": arguments["id"]})
            return _text_result(f"Memory not found: {arguments['id']}", is_error=True)

        elif name == "memory_pin":
            value = not arguments.get("unpin", False)
            updated = await mgr.update_memory(arguments["id"], is_pinned=value)
            if updated:
                action = "unpinned" if arguments.get("unpin") else "pinned"
                return _json_result({"status": action, "id": arguments["id"]})
            return _text_result(f"Memory not found: {arguments['id']}", is_error=True)

        elif name == "memory_delete":
            deleted: bool = await mgr.delete_memory(arguments["id"])
            if deleted:
                return _json_result({"status": "deleted", "id": arguments["id"]})
            return _text_result(f"Memory not found: {arguments['id']}", is_error=True)

        elif name == "memory_tag":
            full_id: Optional[str] = await mgr._resolve_uuid(arguments["id"])
            if not full_id:
                return _text_result(f"Memory not found: {arguments['id']}", is_error=True)
            tag_names: list[str] = arguments["tags"]
            if arguments.get("remove", False):
                ...
                return _json_result({"status": "tags_removed", "id": full_id, "tags": tag_names})
            else:
                for tag_name in tag_names:
                    await mgr.pool.execute(
                        """
                ...
                return _json_result({"status": "tags_added", "id": full_id, "tags": tag_names})
            ...
            memories = await mgr.list_memories(
                include_archived=include_archived, limit=100000,
            )
            return _json_result(memories)

        elif name == "memory_import":
            mem_list: list[dict] = arguments["memories"]
            count: int = 0
            for mem_data in mem_list:
                await mgr.add_memory(
                    ...
                    subcategory=mem_data.get("subcategory"),
                ...
                count += 1
            return _json_result({"status": "imported", "count": count})

        elif name == "memory_prune":
            do_delete: bool = arguments.get("delete", False)
            ...
            if expired_count == 0:
                return _json_result({"status": "nothing_to_prune", "count": 0})

            if dry_run:
                rows = await mgr.pool.fetch(
                    "SELECT id, summary, expires_at FROM memories "
                    "WHERE expires_at IS NOT NULL AND expires_at < CURRENT_TIMESTAMP "
                    "AND is_archived = FALSE ORDER BY expires_at ASC"
                )
                preview: list[dict] = [
                    {"id": str(r["id"]), "summary": r["summary"], "expires_at": r["expires_at"]}
                    for r in rows
                ]
                return _json_result({"status": "dry_run", "count": len(preview), "memories": preview})

            if do_delete:
                result_str: str = await mgr.pool.execute(
                    "DELETE FROM memories "
                    "WHERE expires_at IS NOT NULL AND expires_at < CURRENT_TIMESTAMP"
                )
                count = int(result_str.split(" ")[-1])
                return _json_result({"status": "deleted", "count": count})
            else:
                result_str = await mgr.pool.execute(
                    "UPDATE memories SET is_archived = TRUE "
                    "WHERE expires_at IS NOT NULL AND expires_at < CURRENT_TIMESTAMP "
                    "AND is_archived = FALSE"
                )
                count = int(result_str.split(" ")[-1])
                return _json_result({"status": "archived", "count": count})

        elif name == "memory_backfill":
            include_archived = arguments.get("include_archived", False)
            conditions: list[str] = ["embedding IS NULL"]
            if not include_archived:
                conditions.append("is_archived = FALSE")
            ...
            total: int = len(rows)
            if total == 0:
                return _json_result({"status": "complete", "message": "All memories already have embeddings"})
            success: int = 0
            failed: int = 0
            for row in rows:
                embed_text: str = f"{row['summary'] or ''} {row['content']}"
                ...
                if embedding:
                    ...
                    failed += 1
            return _json_result({"status": "complete", "total": total, "embedded": success, "failed": failed})

        elif name == "memory_bootstrap":
            fmt: str = arguments.get("format", "text")
            recent_limit: int = arguments.get("limit", 5)
            seen_ids: set[str] = set()
            sections: list[tuple[str, list[dict]]] = []
            ...
            sections.append(("CRITICAL", critical))
            for cat in ("project", "technical", "learning", "workflow"):
                recent: list[dict] = await mgr.list_memories(category=cat, limit=recent_limit)
                sections.append((f"RECENT ({cat})", recent))

            if fmt == "json":
                result_data: dict = {"sections": {}, "total": 0}
                total_count: int = 0
                for section_name, memories in sections:
                    entries: list[dict] = []
                    for mem in memories:
                        ...
                        total_count += 1
                    if entries:
                        result_data["sections"][section_name] = entries
                result_data["total"] = total_count
                return _json_result(result_data)
            else:
                output_lines: list[str] = ["=== AGENT MEMORIES BOOTSTRAP ==="]
                total_count = 0
                for section_name, memories in sections:
                    lines: list[str] = []
                    for mem in memories:
                    ...
                    if lines:
                        ...
                        output_lines.extend(lines)
                if total_count == 0:
                    output_lines.append("No memories stored yet.")
                else:
                    output_lines.insert(1, f"[{total_count} memories injected]")
                output_lines.append("\n=== END BOOTSTRAP ===")
                return _text_result("\n".join(output_lines))

        elif name == "memory_bootstrap_slim":
            fmt: str = arguments.get("format", "text")
            seen_ids: set[str] = set()
            sections: list[tuple[str, list[dict]]] = []
            ...
            if fmt == "json":
                ...
                output_lines: list[str] = ["=== AGENT MEMORIES BOOTSTRAP (SLIM) ==="]
                total_count = 0
                for section_name, memories in sections:
                    lines: list[str] = []
                    for mem in memories:
...
async def cmd_mcp(mgr: MemoryManager, _args: argparse.Namespace) -> None:
    """
    Run the MCP stdio server.

    Reads newline-delimited JSON-RPC 2.0 messages from stdin,
    dispatches to tool handlers, writes responses to stdout.
    """
    tools: list[dict] = _mcp_tool_definitions()
    reader = asyncio.StreamReader()
    protocol = asyncio.StreamReaderProtocol(reader)
    await asyncio.get_running_loop().connect_read_pipe(lambda: protocol, sys.stdin.buffer)

    def _write(msg: dict) -> None:
        line: str = json.dumps(msg, default=_mcp_serialize) + "\n"
    ...
    while True:
        try:
            ...
            if not line_bytes:
                break  # EOF
            line_str: str = line_bytes.decode("utf-8").strip()
            if not line_str:
                continue

            msg: dict = json.loads(line_str)
        except (json.JSONDecodeError, UnicodeDecodeError) as e:
            _write(_error(None, -32700, f"Parse error: {e}"))
            continue
        ...
        if method == "initialize":
            ...
            result: dict = await _mcp_handle_tool_call(mgr, tool_name, tool_args)
            _write(_response(req_id, result))
...
examples:
  ...
  agent_memories bootstrap
  agent_memories bootstrap --format json --limit 10
...
importance: """ + ", ".join(IMPORTANCE_LEVELS),
    ...
    p_add.add_argument("--expires", type=str, default=None,
                        help="Expiry: relative (7d, 2w, 3m, 1y) or ISO date")
    ...
    p_search.add_argument("--semantic", action="store_true",
                          help="Use semantic similarity search via pgvector + ollama embeddings")
    ...
    p_prune = subparsers.add_parser(
        "prune",
        help="Archive or delete expired memories",
    )
    p_prune.add_argument("--delete", action="store_true",
                          help="Permanently delete expired memories instead of archiving")
    ...
    p_backfill = subparsers.add_parser(
        "backfill",
        help="Generate embeddings for memories missing them (requires ollama)",
    )
    p_backfill.add_argument("--include-archived", action="store_true",
                             help="Include archived memories in backfill")

    # --- bootstrap ---
    p_bootstrap = subparsers.add_parser(
        "bootstrap",
        help="Output structured context for AI session bootstrap hooks",
    )
    ...
    p_bootstrap.add_argument(
        ...
        help="Number of recent memories per category (default: 5)",
    )

    # --- mcp ---
    subparsers.add_parser(
        "mcp",
        help="Run MCP stdio server (Model Context Protocol over JSON-RPC 2.0)",
    )
